...
def AskQuestion(data):
    while True:
        i = next((i for i, item in enumerate(data) if item["confidence"] == 0), None)
        print(i)
        time.sleep(60)

def Input(data, nodes):
    while True:
        message = input("User : ") 
...
def main():
    ...
    Thread(target = Input(data,nodes)).start()
    while True:
        if time.time() - stime >= 60:
            stime = time.time()
            Thread(target = AskQuestion(data)).start()
            Thread(target = Input(data,nodes)).join()
            time.sleep(5)
            Thread(target = AskQuestion(data)).joint()
            Thread(target = Input(data,nodes)).start()
